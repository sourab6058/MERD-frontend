        ...
        if (this.props.shouldreload === 'true') {
            return (
                <SubMenu {...filteredProps} title={this.props.sub.name} >
                    <Menu.Item><Checkbox onClick={(e) => this.checkedAll('subCategories', this.props.sub.id, e)} checked={this.props.checked}>Select All</Checkbox></Menu.Item>
                    {this.props.sub.sub_sub_category.map(subsub =>
                        this.state.checkedAll ? <Menu.Item key={subsub.name}><Checkbox onChange={(e) => this.props.additem('subSubCategories', subsub.id, e)} checked={false} disabled={this.state.checkedAll}>{subsub.name}</Checkbox></Menu.Item>
                            : <Menu.Item key={subsub.name}><Checkbox checked={this.props.subsubCatgSelected.includes(subsub.id)} onClick={(e) => this.props.additem('subSubCategories', subsub.id, e)}>{subsub.name}</Checkbox></Menu.Item>)}
                </SubMenu>
            )
        } else {
            return (
